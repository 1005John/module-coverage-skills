...
一套完整的蜂窝通信模组覆盖率测试技能，支持从 AT 手册 PDF 到测试报告的端到端流程。
...
```
AT手册(PDF) → 手册知识库 → 模块模型 → 用例生成 → 插桩 → 编译 → 烧录 → 测试执行 → 分析迭代 → 报告
```
...
| `coverage-build-flash` | 固件编译（增量）和烧录 | 编译/烧录 |
...
支持 `AT+COVERAGE=2..9` 输出 bitmap 分块，精确计算每个 stub 是否命中：
